...
const data2 = (data as any[]).reduce((acc, el) => {
  const commune = {
    id: el.id,
    name: el.name,
    localities: (el.localities as any[]).map((value) => ({
      id: value.id,
      name: value.location,
    })),
  };
...
const jsonData = JSON.stringify(Object.values(data2), null, 2); // `null, 2` es para dar formato legible con indentación
fs.writeFile('localities.json', jsonData, (err) => {
  if (err) {
